...
# hmu by our method
hmu = a[["mu_s"]] + a[["mu_w"]]
setB = which(-0.2 <= hmu & hmu <= 0.2)
# hmu by the adaptive method
hmu_adp = X2 %*% c$hbeta

# target
target = hmu
target[setB] = hmu[setB] + 2.5 * (hmu_adp[setB] - hmu[setB])
...
(t(part1) %*% part2 %*% part1)/ n2
(t(part1) %*% part2 %*% part1)/ n2 <= 1

# for object "c"
L2NormSquare(target - X2 %*% c$hbeta) / n2 / (c$r^2)
L2NormSquare(target - X2 %*% c$hbeta) / n2 / (c$r^2) <= 1

# sample
IsB = rep("No", n2)
IsB[setB] = "Yes"
df = rbind(data.frame(index = factor(which(sub_grp == 2)), vec = target, label = "mu", InSetB = IsB),
           data.frame(index = factor(which(sub_grp == 2)), vec = Y2, label = "Y", InSetB = IsB))

ggplot(data = df) + geom_point(aes(x = index, y = vec, color = label, shape = InSetB), alpha = 0.5, size = 3) +
...
# target
target = hmu
target[setB] = hmu[setB] + 5.5 * (hmu_adp[setB] - hmu[setB])
...
(t(part1) %*% part2 %*% part1)/ n1
(t(part1) %*% part2 %*% part1)/ n1 <= 1

# for object "d"
L2NormSquare(target - X1 %*% d$hbeta) / n1 / (d$r^2)
L2NormSquare(target - X1 %*% d$hbeta) / n1 / (d$r^2) <= 1

IsB = rep("No", n1)
IsB[setB] = "Yes"
df = rbind(data.frame(index = factor(which(sub_grp == 1)), vec = target, label = "mu", InSetB = IsB),
           data.frame(index = factor(which(sub_grp == 1)), vec = Y1, label = "Y", InSetB = IsB))
ggplot(data = df) + geom_point(aes(x = index, y = vec, color = label, shape = InSetB), alpha = 0.5, size = 3) +
...
####################### increase min{|y_i|} to max{|y_i|} group1 ########################
...
####################### increase min{|y_i|} to max{|y_i|} group2 ########################
...
for (i in 1:tot) {
  ...
  X1 = x[index, ]
  Y1 = matrix(y[index], ncol = 1)
  X2 = x[-index, ]
  Y2 = matrix(y[-index], ncol = 1)
  # group 1
  cv_model = cv.glmnet(X1, Y1, intercept = F)
  lam1 = cv_model$lambda.1se
  ste1 = two_step(X1, X2, Y1, Y2, lam1, numCandidate = 0, alpha = 0.05, upper = 3, ite = 500)
  ste1 = ste1$volume_stein
  adp1 = adaptive(X1, Y1, X2, Y2, Y2, lam1, sig = 1, alpha = 0.05)
  r_s1[i] = ste1$r_s
  r_w1[i] = ste1$r_w
  logV1[i] = ste1$logVol
  k1[i] = ste1$k
  ...
  cv_model = cv.glmnet(X2, Y2, intercept = F)
  lam2 = cv_model$lambda.1se
  ste2 = two_step(X2, X1, Y2, Y1, lam2, numCandidate = 0, alpha = 0.05, upper = 3, ite = 500)
  ste2 = ste2$volume_stein
  adp2 = adaptive(X2, Y2, X1, Y1, Y1, lam2, sig = 1, alpha = 0.05)
  r_s2[i] = ste2$r_s
  r_w2[i] = ste2$r_w
  logV2[i] = ste2$logVol
  k2[i] = ste2$k
...
#####
# adaptive
df_adp = data.frame(sample = factor(which(sub_grp == 2)), lower = as.vector(X2 %*% c$hbeta - c$r), 
                    upper = as.vector(X2 %*% c$hbeta + c$r), center = as.vector(X2 %*% c$hbeta))
f1_adp = ggplot(df_adp, aes(x = sample)) + geom_errorbar(aes(x = sample, ymin = lower, ymax = upper)) +
  geom_point(aes(y = center)) + 
  geom_hline(yintercept = 0, linetype = 3) +
  guides(color = FALSE) + ylab('confidence band') + xlab('individual index') +
  ggtitle("(a) group 1") + theme2
...
a = two_step_stein_real_data(x, y, sig, index) 
a$coverage
