...
two_step = function(X1, X2, Y1, Y2, lam1, lam2Coef, s, sig, ratios, alpha, upper, ite)
{
  ...
  p = dim(X1)[2]
  # stein c(alpha)
  stein = SteinToZero(Y2, n2, sig)
  c_alpha_stein = quantile_stein(stein$mu, n2, alpha / 2, sig, ite)
  # lasso c(alpha)
  ...
  logVol_volume_lasso = rep(0, res_candidate[['count']])
  logVol_radius_lasso = rep(0, res_candidate[['count']])

  # record
  for (i in 1:res_candidate[['count']])
  {
    res_stein = weak_stein(X2, Y2, res_candidate[['candidates']][[i]], sig, alpha / 2, c_alpha_stein, upper)
    res_lasso = weak_lasso(X2, Y2, res_candidate[['candidates']][[i]], sig, alpha / 2, c_alpha_lasso, lam2Coef, s, upper)
  ...
  volume_lasso = list(r_s = res_lasso_pool[[i_volume_lasso]][['r_s_volume']],
                      ...
                      A = res_candidate[['candidates']][[i_volume_lasso]],
                      k = length(res_candidate[['candidates']][[i_volume_lasso]]),
                      tau = res_candidate[['tau_chosen']][[i_volume_lasso]])
  ...
  radius_lasso = list(r_s = res_lasso_pool[[i_radius_lasso]][['r_radius']],
                      ...
                      A = res_candidate[['candidates']][[i_radius_lasso]],
                      k = length(res_candidate[['candidates']][[i_radius_lasso]]),
                      tau = res_candidate[['tau_chosen']][[i_radius_lasso]])
