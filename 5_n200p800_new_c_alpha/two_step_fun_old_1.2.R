...
weak_stein = function(X2, Y2, A, sig, alpha_s, c_alpha_w, upper)
{
  ...
  {
    P_A = B2P(qr_decom$qr[,1:qr_decom$rank])
...
threshold = function(beta, tau, X2) 
{
  count = 0
  candidates = list()
  tau_chosen = list()
...
two_step = function(X1, X2, Y1, Y2, lam1, lam2Coef, s, sig, ratios, alpha, upper, ite)
{
  ...
  res_candidate = threshold(as.matrix(lasso_split$beta), lam1 * ratios, X2)
  # trunate beta to s-1 nonzero coefs, so |A| <= s-1
  res_candidate_lasso = threshold(trunate_beta(as.matrix(lasso_split$beta), s - 1), lam1 * ratios, X2)
  ...
  volume_lasso = list(r_s = res_lasso_pool[[i_volume_lasso]][['r_s_volume']],
                      ...
                      tau = res_candidate_lasso[['tau_chosen']][[i_volume_lasso]])
  ...
  radius_lasso = list(r_s = res_lasso_pool[[i_radius_lasso]][['r_radius']],
                      ...
                      tau = res_candidate_lasso[['tau_chosen']][[i_radius_lasso]])
                           ...
                           lam2Coef, ratios, upper)
{
  ...
  for (i in 1:length(lam1Types))
  {
    for (j in 1:length((svalue)))
    {
      ...
      {
        print(lam1Types[i], svalue[j], bvalue[k])
        arg[['root']] = rawDir
