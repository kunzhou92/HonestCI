...
res_multiple = list()
for (i in 1:length(dir_pool)) {
  temp = oracle_single(2 * N, P, SVALUE, BVALUE, sig = SIG, lam1Types_oracle, alpha, ITE, dir_pool[i])
  temp['design'] = design[i]
  res_single[[i]] = temp
...
for (i in 1:length(dir_pool)) {
  temp = adjust_lasso_single_version2(2 * N, P, SVALUE, BVALUE, SIG, ITE, dir_pool[i],
...
save_list2 = c("summary_adaptive_single", "summary_adaptive_multiple",
               "summary_oracle_single", "summary_oracle_multiple",
               "summary_volume_stein_single", "summary_radius_stein_single",
               "summary_volume_lasso_single", "summary_radius_lasso_single",
               "summary_volume_stein_multiple", "summary_radius_stein_multiple",
...
save(list = save_list2, file = 'summary2.Rdata')