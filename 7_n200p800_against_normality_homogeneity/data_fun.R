...
# note that sigma is not constant for this script
  ...
  for (j in 1:length(svalue))
  {
    for (k in 1:length(bvalue))
    {
      ...
      for (i in 1:ite)
      {
        ...
        betaMat[i,] = t(beta)
        ####################### Y1 and Y2 should be changed simultaneously ########
        mu1 = as.vector(tcrossprod(X1, t(beta)))
        mu2 = as.vector(tcrossprod(X2, t(beta)))
        lo = quantile(c(mu1, mu2), 0.05)
        hi = quantile(c(mu1, mu2), 0.95)
        incre = 9
        coef = incre / (hi - lo)^2
        ...
        Y1 = tcrossprod(X1, t(beta)) + matrix(sigma_var1 * rnorm(n), ncol = 1)
        
        # Y1 = tcrossprod(X1, t(beta)) + matrix(sig * rt(n, df = 1), ncol = 1)
        Y1Mat[i,] = Y1
        # Y2Mat[i,] = tcrossprod(X2, t(beta)) + matrix(sig * rt(n, df = 1), ncol = 1)
        Y2Mat[i,] = tcrossprod(X2, t(beta)) + matrix(sigma_var2 * rnorm(n), ncol = 1)
