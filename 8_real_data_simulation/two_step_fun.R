...
two_step = function(X1, X2, Y1, Y2, lam1, lam2Coef, s, sig, numCandidate, alpha, upper, ite)
{
  ...
  ################################# new part, modify it so that candidates won't exceed n2 ###########################
  candidates = list()
  if (sum(lasso_split$beta != 0) <= n2) {
    candidates[[1]] = which(lasso_split$beta != 0)
  } else {
    hbeta_tmp = abs(as.numeric(lasso_split$beta))
    candidates[[1]] = order(hbeta_tmp, decreasing = T)[1:n2]
  }
  res_candidate = list(candidates = candidates,
                       count = 1)
  ...
  ################################# new part ###########################
  # this part is for lasso
  candidates = list()
  candidates[[1]] = integer(0)
  res_candidate_lasso = list(candidates = candidates,
                             count = 1)
  ########################################################################
  
  ################# new estimate c_s(\alpha) #####################
...
convert = function(m_list, X2, beta) {
  ...
  else
  {
    ...
    ######################## new part #################
    # P_A = B2P(X2[,m_list[['A']]]) should be replaced by the following
...
two_step_test = function(n1, n2, p, sig, alpha, ite, arg)
{
  ite_quantile = 100
  ...
  if (arg[['lam1Type']] == '1se') {
    ...
    ######################### new part #####################################
      ...
      lam1 = 2 * sqrt(2) * sqrt(log(p) / n1) * hsigma[,1]
  }
...
two_step_single = function(n1, n2, p, svalue, bvalue, sig, lam1Types, alpha, ite, rawDir, 
  ...
  for (i in 1:length(lam1Types))
  {
    for (j in 1:length((svalue)))
    {
      for (k in 1:length(bvalue))
      {
        ...
        tempRes = two_step_test(n1, n2, p, sig, alpha, ite, arg)
