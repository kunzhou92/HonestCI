...
adaptive_test = function(n1, n2, p, sig, alpha, ite, arg)
{
  r = rep(0, ite)
  coverage = rep(0, ite)
  logVol = rep(0, ite)
  ...
  for(i in 1:ite)
  {
    ...
    if (arg[['lam1Type']] == '1se') {
      ...
      lam1 = rep(2 * sqrt(2) * sqrt(log(p) / n1) * sig, ite)
    }
    ###########################################################################
...
adaptive_single = function(n1, n2, p, svalue, bvalue, sig, lam1Types, alpha, ite, rawDir)
{
  arg = list()
  res_list = list()
  count = 1
  for (i in 1:length(lam1Types))
  {
    for (j in 1:length((svalue)))
    {
      for (k in 1:length(bvalue))
      {
        ...
        tempRes = adaptive_test(n1, n2, p, sig, alpha, ite, arg)
